...
function applyRoutes(server, call) {
    ...
    server.route({
        ...
        path: '/admin/config/license',
        async handler(request, h) {
            await request.flash({ type: 'info', message: `License key management is not used in SSPL-only mode` });
        ...
        options: {
            validate: {
                ...
                async failAction(request, h, err) {
                    logger.error({ msg: 'Failed to process request for /admin/config/license', err });
                    await request.flash({ type: 'danger', message: `Failed to process request` });
    ...
    server.route({
        ...
        options: {
            validate: {
                ...
                async failAction(request, h, err) {
                    logger.error({ msg: 'Failed to process request for /admin/config/license/delete', err });
                    await request.flash({ type: 'danger', message: `Failed to process request` });
    ...
    server.route({
        ...
        async handler(request) {
            return { success: false, error: 'Trial licensing is not available in SSPL-only mode' };
        },
        options: {
            validate: {
                options: {
