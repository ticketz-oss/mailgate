...
const licenseSchema = Joi.object({
    active: Joi.boolean().example(true).description('Current license status flag'),
    type: Joi.string().example('SSPL-1.0').description('Current license type'),
    details: Joi.object({
        application: Joi.string().example('@mailgate/app'),
        key: Joi.string().hex().example('1edf01e35e75ed3425808eba').description('License identifier if available'),
        licensedTo: Joi.string().hex().example('Kreata OÜ').description('License owner if available'),
        hostname: Joi.string().example('emailengine.example.com').description('Bound hostname/environment if available'),
        created: Joi.date().example('2021-10-13T07:47:42.695Z').description('Creation time if available')
    })
        .allow(false)
